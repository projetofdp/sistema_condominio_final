...
def atualizar_dados_canvas(canvas,dados_visitantes_antigos):
    # Limpa apenas os elementos dinâmicos do canvas
    for item in canvas.find_withtag("dinamico"):
        canvas.delete(item)

    for i, dado in enumerate(dados_visitantes_antigos):
        nome = dado[0]
        hora1 = dado[1]
        hora2 = dado[2]
        data = dado[3]
...
if len(sys.argv) > 1:
    dados_visitantes_antigos_json = sys.argv[1]
    try:
        dados_visitantes_antigos = json.loads(dados_visitantes_antigos_json)
        atualizar_dados_canvas(canvas, dados_visitantes_antigos)
