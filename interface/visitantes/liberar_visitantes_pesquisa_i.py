...
def atualizar_dados_canvas(canvas,dados_visitantes):
    ...
    for i, dado in enumerate(dados_visitantes):
        nome = dado[0]
        hora1 = dado[1]
        hora2 = dado[2]
        data = dado[3]
...
if len(sys.argv) > 1:
    dados_visitantes_json = sys.argv[1]
    try:
        dados_visitantes = json.loads(dados_visitantes_json)
        atualizar_dados_canvas(canvas, dados_visitantes)
