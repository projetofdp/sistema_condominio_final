...
def verificaçao_importaçao():
# Verifica se há argumentos passados
    if len(sys.argv) < 2:
        print("Nenhum dado recebido.")
    else:
        # Obter os dados JSON passados como argumento
        ...
        try:
            ...
            print("Dados recebidos:", dados_encomenda)
            
            # Processar os dados conforme necessário
            # Por exemplo, você pode imprimir os dados ou realizar outras operações
            for encomenda in dados_encomenda["encomendas"]:
                print(f"Data: {encomenda['data']}, Descrição: {encomenda['descricao']}")
...
def atualizar_dados_canvas(canvas, resultados):
    ...
    for i, dado in enumerate(resultados[:3]):  # Limita a três resultados
        ...
        encomenda = {
            ...
            'Id': Id,
            'porteiro': porteiro,
            'nome_de_quem_retirou': nome_de_quem_retirou
        }
        if i == 0:
            preencher_encomenda1(encomenda)
        elif i == 1:
            preencher_encomenda2(encomenda)
        elif i == 2:
            preencher_encomenda3(encomenda)
...
def preencher_encomenda1(nome, data_entrega, data_retirada, bloco, apartamento, Id, porteiro,nome_de_quem_retirou):
    ...
    canvas.create_text( 114.0, 228.0, anchor="nw", text=nome, fill="#000000", font=("BeVietnamPro Medium", 14 * -1), tags="dinamico")
